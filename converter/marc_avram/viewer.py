...
.hunkpath{color:var(--text);font-weight:650}
...
function buildDiff(f){
  ...
  f.lines.forEach(([m,t])=>{
    ...
    const pm=m==="@" ? t.match(/^(@@ .*? @@)( .+)$/) : null;
    if(pm){                       // highlight the JSON-path hunk context
      const txt=el("span","txt",pm[1]);
      txt.appendChild(el("span","hunkpath",pm[2]));
